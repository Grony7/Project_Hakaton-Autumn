...
func UserLoginDb(email string, password string) []User {
	...
	data, err := db.Query(fmt.Sprintf("SELECT `user_id`, `email`, `password`, `role` FROM `users`"+
	...
	for data.Next() {
		var userEnter User
		err := data.Scan(&userEnter.UserId, &userEnter.Email, &userEnter.Password, &userEnter.Role)
		if err != nil {
			panic(err)
		}
